...
impl<'a> Parser<'a> {
    ...
    /// ```
    pub fn register<T: Argument + 'a, F : FnMut(&T) -> () + 'a>(&mut self, names : &[&'a str], argument : T, handle : F) {
        self.arguments.push(argument.into());
        self.callbacks.push(Box::new(
            Callback::<T, F> {
                phantom : PhantomData,
                callback : handle
            }
    ...
    pub fn parse<S : Scanner>(&mut self, scanner : S) -> Result {
        ...
        for token in scanner {
            ...
            if add_to_free_variables {
                ...
                // If a argument waits for its value ...
                    ...
                    unsafe {
                        if let &mut Arguments::Value(ref mut value) = self.arguments.get_unchecked_mut(value_id) {
        ...
        for (index, callback) in self.callbacks.iter_mut().enumerate() {
            callback.execute(unsafe {
                self.arguments.get_unchecked(index)
            })
        }
